...
void ReheapDown(PQueue* pQueue, int parentIndex)
{
	...
	{
		if (pQueue->items[leftChildIndex]->income >
			pQueue->items[maxIndex]->income)
			maxIndex = leftChildIndex;
		if (rightChildIndex < pQueue->currentIndex &&
			pQueue->items[rightChildIndex]->income >
			pQueue->items[maxIndex]->income)
			maxIndex = rightChildIndex;
		if (parentIndex != maxIndex)
		{
			Student* stud = pQueue->items[maxIndex];
			pQueue->items[maxIndex] = pQueue->items[parentIndex];
			pQueue->items[parentIndex] = stud;
